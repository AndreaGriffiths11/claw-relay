...
| Tiny/squished viewport | Restart the relay — the viewport fix requires a fresh server start. `Ctrl+C` then `npx @acolombiadev/claw-relay` |
...
| Problem | Fix |
|---------|-----|
| `502` from tunnel | The relay server isn't running. Start it with `npx @acolombiadev/claw-relay` |
